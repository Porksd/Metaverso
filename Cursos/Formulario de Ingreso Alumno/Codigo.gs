// ==============================
// Code.gs — COMPLETO (Tiempo Total + MM:SS para listado) + signal robusto
// ==============================
...
const SPREADSHEET_ID = '1vRRtA6M9_53S_0FNyJs8j1bBK9LTiE1bDr0PqX-tzao';
 ...
 * Reforzado con fallbacks cross-origin: window.name y location.hash.
 */
function signalHtml_(dest) {
  ...
  var html =
    ...
    // --- 1) Difundir por postMessage (si hay parent/opener)
    '   function sendTo(win){ try{ if(win) win.postMessage(msg,"*"); }catch(e){} }' +
    '   function broadcastPM(){' +
    '     sendTo(window.parent); sendTo(window.top);' +
    '     if (window.opener){' +
    '       sendTo(window.opener);' +
    '       try{ sendTo(window.opener.parent); }catch(e){}' +
    '       try{ sendTo(window.opener.top); }catch(e){}' +
    '       try{ if (window.opener.parent && window.opener.parent.parent) sendTo(window.opener.parent.parent); }catch(e){}' +
    '     }' +
    '   }' +

    // --- 2) BroadcastChannel (no requiere opener)
    '   try{' +
    '     var bc = new BroadcastChannel("genially-signal");' +
    '     bc.postMessage({type:"fin", dest:dest, ts:Date.now()});' +
    '     setTimeout(function(){ try{ bc.close(); }catch(e){} }, 500);' +
    '   }catch(e){}' +

    // --- 3) localStorage (dispara evento storage en el index)
    '   try{ localStorage.setItem("geniallySignal", JSON.stringify({type:"fin", dest:dest, ts:Date.now()})); }catch(e){}' +

    // --- 4) Fallbacks visibles por el padre si está embebido
    '   try{ window.name = msg; }catch(e){}' +
    '   try{ location.hash = "#fin:" + (dest||""); }catch(e){}' +

    '   var n=0, id=setInterval(function(){ broadcastPM(); if(++n>40){ clearInterval(id); try{ window.close(); }catch(e){} } }, 150);' +
    '   broadcastPM();' +
    ' })();' +
    '<\\/script>';
...
function doGet(e) {
  ...
  return HtmlService.createHtmlOutputFromFile("index")
    .setTitle("PREVENCIÓN DE INCENDIOS")
    .setXFrameOptionsMode(HtmlService.XFrameOptionsMode.ALLOWALL);
}
...
function guardarDatos(data) {
  ...
  if (!yaEnviado && terminado) {
    ...
    try {
      ...
      props.setProperty(keySent, '1'); // marca como enviado
      enviado = true;
