...
if ( ! class_exists( '\app\wisdmlabs\edwiserBridgePro\includes\bulkPurchase\Eb_Bp_Manage_Groups' ) ) {

	/**
	 * Manage Groups.
	 */
	class Eb_Bp_Manage_Groups {
		 ...
		 *
		 * @var Eb_Bp_Manage_Groups The single instance of the class
		 *
		 * @since 2.3.8
		 */
		...
		/**
		 * Main Eb_Bp_Manage_Groups Instance.
		 *
		 * Ensures only one instance of Eb_Bp_Manage_Groups is loaded or can be loaded.
		 *
		 * @since 2.3.8
		 * @static
		 *
		 * @see Eb_Bp_Manage_Groups()
		 *
		 * @param text $plugin_name plgin name.
		 * @param text $version plgin version.
		 * @return Eb_Bp_Manage_Groups - Main instance
		 */
		...
		public function __clone() {
			_doing_it_wrong( __FUNCTION__, esc_html__( 'Cheatin&#8217; huh?', 'edwiser-bridge-pro' ), '2.3.8' );
		}
		...
		public function output() {
			$list_table     = new Eb_Bp_Group_Table();
			...
			<div class="eb-manage-user-enrol-wrap">
				...
				<div id="loading-div-background">
					<div id="loading-div" class="ui-corner-all" >
						...
						<h2 style="color:gray;font-weight:normal;">
							<?php esc_html_e( 'Please wait processing request ....', 'edwiser-bridge-pro' ); ?>
						</h2>
					</div>
				</div>
				...
				<p class='search-box'>
					<input type="text" id="ebbp_grp_search" name="ebbp_grp_search" value="<?php echo esc_html( $search_text ); ?>">
					<input type="submit" name="eb_manage_enroll_search" id="eb_manage_enroll_search" class="button action" value="<?php echo esc_html__( 'Search Groups', 'edwiser-bridge-pro' ); ?>"/>
				</p>
					...
					<?php
					wp_nonce_field( 'eb-bp-manage-groups', 'eb-bp-manage-groups' );
		...
		private function handle_bulk_action( $action ) {
			if ( ! isset( $_POST['eb-bp-manage-groups'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['eb-bp-manage-groups'] ) ), 'eb-bp-manage-groups' ) ) {
				$post_data = array();
			} else {
				$post_data = $_POST;
			}
		...
		private function multiple_unenroll_by_rec_id( $data ) {
			...
			if ( $cnt > 0 ) {
				...
				<div class="notice notice-success is-dismissible">
					<p>
						<strong>
							<?php sprintf( '%s ', $cnt ) . esc_html_e( 'users has been unenrolled successfully.', 'edwiser-bridge-pro' ); ?>
						</strong>
					</p>
					<button type="button" class="notice-dismiss">
						<span class="screen-reader-text">
						<?php
						esc_html_e( 'Dismiss this notice', 'edwiser-bridge-pro' );
						?>
						.</span>
					</button>
				</div>
				...
				<div class="error notice">
					<p>
						<strong>
							<?php esc_html_e( 'No users has been unenrolled', 'edwiser-bridge-pro' ); ?>
						</strong>
					</p>
					<button type="button" class="notice-dismiss">
						<span class="screen-reader-text">
		...
		public function unenroll_user_ajax_handler() {
			$response = esc_html__( 'Failed to unenroll user', 'edwiser-bridge-pro' );
			if ( isset( $_POST['user_id'] ) && isset( $_POST['course_id'] ) && isset( $_POST['action'] ) && 'wdm_eb_user_manage_unenroll_unenroll_user' === $_POST['action'] && isset( $_POST['admin_nonce'] ) && wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['admin_nonce'] ) ), 'eb_admin_nonce' ) ) {
				...
				if ( $res ) {
					...
					$response    = ucfirst( $user->user_login ) . esc_html__( ' has been unenrolled from the ', 'edwiser-bridge-pro' ) . $course_name . esc_html__( ' course', 'edwiser-bridge-pro' );
		...
		private function unenroll_user( $course_id, $user_id ) {
			...
			$enrollment_manager = new Eb_Bp_Enrollment_Manager( $this->plugin_name, $this->version );
