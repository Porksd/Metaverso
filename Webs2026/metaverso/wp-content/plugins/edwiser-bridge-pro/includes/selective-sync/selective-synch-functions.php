...
if ( ! function_exists( 'eb_ss_get_moodle_users_in_chunk' ) ) {
	...
	function eb_ss_get_moodle_users_in_chunk( $offset, $limit, $search_text ) {
		...
		$total_users    = 0;
		$users          = array();
		$web_service_fn = 'auth_edwiserbridge_get_users';
		$request_data   = array(
			'offset'        => $offset,
			'limit'         => $limit,
		...
		$result         = \app\wisdmlabs\edwiserBridge\edwiser_bridge_instance()->connection_helper()->connect_moodle_with_args_helper( $web_service_fn, $request_data );
...
if ( ! function_exists( 'eb_ss_test_connection' ) ) {
	...
	function eb_ss_test_connection() {
		...
		$connected = \app\wisdmlabs\edwiserBridge\edwiser_bridge_instance()->connection_helper()->connection_test_helper( $eb_moodle_url, $eb_moodle_token );
...
if ( ! function_exists( 'eb_ss_get_users_status' ) ) {
	...
	function eb_ss_get_users_status( $email ) {
		...
		$status  = __( 'Not Created', 'edwiser-bridge-pro' );

		if ( $user_id ) {
			$status         = __( 'Created', 'edwiser-bridge-pro' );
			$moodle_user_id = get_user_meta( $user_id, 'moodle_user_id', true );

			if ( $moodle_user_id ) {
				$status = __( 'Linked', 'edwiser-bridge-pro' );
			}
		}
