...
/**
 * SendProviderBookingConfirmationEmailHandler
 */
class SendProviderBookingConfirmationEmailHandler implements HandlerInterface
{
    public function dispatch(CommandInterface $command): void
    {
        /** @var $command SendProviderBookingConfirmationEmail */
        ...
        $send_attachments = ServiceProviderCustomData::provideByWithDefault([
            'service_id'  => $reservation->serviceId,
            'provider_id' => $reservation->providerId,
            'key'         => Personal_ConfirmationEmailToProvider::ID_ATTACHMENTS
        ], $service);

        $subject_raw = ServiceProviderCustomData::provideByWithDefault([
            'service_id'  => $reservation->serviceId,
            'provider_id' => $reservation->providerId,
            'key'         => Personal_ConfirmationEmailToProvider::ID_SUBJECT
        ], $service);
        ...
        $preparedValues = Notifications::prepare_placeholders($command->getReservationId(), Personal_ConfirmationEmailToProvider::ID);
        ...
        do_action('tbk_send_provider_booking_confirmation_email', $reservation, $send_command);
