...
abstract class Add_On extends \WC_Data {
	...
	public const PERCENTAGE_ADJUSTMENT_SUBTOTAL = 'woocommerce_checkout_order_subtotal';

	/** @var string apply percentage adjustment on cart total */
	public const PERCENTAGE_ADJUSTMENT_TOTAL = 'woocommerce_checkout_order_total';
	...
	protected string $add_on_classname = '';
	 ...
	 */
	public function get_cost_including_tax( $cost = null ) : float
	{
		$cost ??= $this->get_adjustment();
		$cost = (float) $cost;

		if ( $this->is_taxable() ) {
			$add_on_taxes = WC_Tax::calc_tax( $cost, $this->get_applicable_tax_rates(), false );
	...
	/**
	 * Gets applicable tax rates within the context of checkout details.
	 *
	 * @since 2.7.2
	 *
	 * @return array
	 */
	public function get_applicable_tax_rates() : array
	{
		// Determine tax rates based on the current customer details
		if ( $customer = WC()->customer ) {
			return array_map( static fn ( $rate ) => [
				'rate'     => (float) $rate['rate'],
				'label'    => $rate['label'],
				'shipping' => wc_bool_to_string( $rate['shipping'] ),
				'compound' => wc_bool_to_string( $rate['compound'] ),
			], WC_Tax::get_rates( $this->get_tax_class(), $customer ),
			);
		}

		// otherwise determine tax rates based on tax class only
		return array_map( static fn ( $rate ) => [
			'rate'     => (float) $rate->tax_rate,
			'label'    => $rate->tax_rate_name,
			'shipping' => wc_bool_to_string( $rate->tax_rate_shipping ),
			'compound' => wc_bool_to_string( $rate->tax_rate_compound ),
		], WC_Tax::get_rates_for_tax_class( $this->get_tax_class() )
		);
	}
	...
	public function get_cost_html( $cost = null, $cost_type = null ) {
		...
		$display_cost = $this->getDisplayCost($cost);
	 ...
	 * @return string|float
	 */
	protected function getDisplayCost($cost)
	{
		return 'incl' === static::getCartTaxPriceDisplayMode() ? $this->get_cost_including_tax($cost) : $cost;
	}


	protected static function getCartTaxPriceDisplayMode() : ?string
	{
		if (! $cart = WC()->cart) {
		...
		return $cart->tax_display_cart ?? null;
	}
	 ...
	 * @return bool
	 */
	public function get_is_taxable ( string $context = 'view' ) : bool
	{
		// If taxes are disabled return false in any case
		if ( 'view' === $context && ! wc_tax_enabled() ) {
			return false;
		}
	...
	{
		$taxClass = $this->get_prop( 'tax_class', $context );

		// make sure to translate standard tax to empty string
		return 'view' === $context && 'standard' === $taxClass ? '' : $taxClass;
	}
	 ...
	 * @return float
	 */
	public function calculate_percentage_adjustment( float $percentage ) : float
	{
		if (! $cart = WC()->cart) {
			return 0.0;
		}

		$apply_percentage_on = get_option(
			'woocommerce_checkout_add_ons_percentage_adjustment_from',
			static::PERCENTAGE_ADJUSTMENT_SUBTOTAL
		);

		$amount = static::PERCENTAGE_ADJUSTMENT_SUBTOTAL === $apply_percentage_on ?
			$cart->get_subtotal() :
