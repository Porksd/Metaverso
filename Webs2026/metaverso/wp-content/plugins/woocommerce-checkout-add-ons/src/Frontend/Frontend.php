...
class Frontend {
	...
	/** @var array add on sanitized titles in order */
	private $add_on_sanitized_titles_in_order;
	...
	public function __construct() {
		...
		// sort add on fees in checkout order review
		add_filter( 'woocommerce_sort_fees_callback', [ $this, 'sort_add_on_fees' ], 10, 3 );
	...
	public function remove_file() {
		...
		if ( ! in_array( $file, $this->wc_session()->checkout_add_ons['files'], false ) ) {
			return;
		}
	 ...
	 * @param \WP_Error $errors validation errors, if present, passed by reference
	 */
	public function validate_checkout_fields( $post_data, $errors ) {
		...
		foreach ( $add_ons as $add_on ) {
			...
			$value = $post_data[ $key ];
			 ...
			 // Checkbox inputs overwrite an hidden input with the same key that exists to allow an unchecked checkbox send an empty value so the preference can saved to the order.
			 // However, required checkboxes need a non-empty value here, hence the additional checks below.
			if ( empty( $value ) && $add_on->is_required() && 'checkbox' === $add_on->get_type() ) {

				if ( ! $errors instanceof \WP_Error ) {
					$errors = new \WP_Error();
				}

				if ( in_array( $key . '_required', $errors->get_error_codes(), false ) ) {
					continue;
				}

				$required_checkbox_notice = sprintf(
					/* translators: Placeholder: %s - Checkout add-on label */
					__( '%s is a required field.', 'woocommerce-checkout-add-ons' ),
					'<strong>' . esc_html( $add_on->get_label() ?? $add_on->get_name() ) . '</strong>'
				);

				/** this filter is documented in WooCommerce core class-wc-checkout.php */
				$errors->add( $key . '_required', apply_filters( 'woocommerce_checkout_required_field_notice', $required_checkbox_notice, [ 'id' => $key ] ) );
			}
	 ...
	 * Sorts add on fees.
	 *
	 * This method is used to ensure the add-on order is respected in the order review area in checkout, and as a result, also in order emails and edit order screen.
	 * @see \WC_Cart_Fees::sort_fees_callback()
	 *
	 * @internal
	 *
	 * @since 2.5.4
	 *
	 * @param int $order sort order identifier (-1 to 1)
	 * @param stdClass $fee_a first fee to compare
	 * @param stdClass $fee_b second fee to compare
	 * @return int
	 */
	public function sort_add_on_fees( int $order, stdClass $fee_a, stdClass $fee_b ) : int {

		$add_ons_in_order = $this->get_add_on_sanitized_titles_in_order();

		if ( in_array( $fee_a->id, $add_ons_in_order, true ) ) {

			if ( in_array( $fee_b->id, $add_ons_in_order, true ) ) {
				$order = array_search( $fee_a->id, $add_ons_in_order, true ) > array_search( $fee_b->id, $add_ons_in_order, true ) ? 1 : -1;
			} else {
				$order = 1;
			}
		}
	...
	/**
	 * Gets add-on sanitized titles in order.
	 *
	 * Memoizes the results, as this method is called multiple times in the checkout.
	 *
	 * @since 2.5.4
	 *
	 * @return array
	 */
	private function get_add_on_sanitized_titles_in_order() : array {

		if ( ! isset( $this->add_on_sanitized_titles_in_order ) || ! is_array( $this->add_on_sanitized_titles_in_order ) ) {

			$this->add_on_sanitized_titles_in_order = array_map( static function( $add_on ) {
				return sanitize_title( $add_on->get_name() );
			}, Add_On_Factory::get_add_ons() );
		}

		return $this->add_on_sanitized_titles_in_order;
	}
	...
	public function adjust_fee_html( $html, $fee ) {
		...
		switch ( $add_on->get_type() ) {
			...
			case 'multicheckbox':
				...
				if ( ! empty( $this->wc_session()->checkout_add_ons['fees'][ sanitize_title( $add_on->get_name() ) ] ) ) {

					$session_data = $this->wc_session()->checkout_add_ons['fees'][ sanitize_title( $add_on->get_name() ) ];
	...
	public function render_add_ons() {
		...
		$checkout_add_on_fields = WC()->checkout()->checkout_fields['add_ons'] ?? null;
		...
		switch ( $args['type'] ) {
			...
			case 'wc_checkout_add_ons_file' :
				...
				if ( $value ) {
					...
					if ( ! in_array( $value, $this->wc_session()->checkout_add_ons['files'], false ) ) {
	...
	public function init_session() : void
	{
		...
		if (! $session->checkout_add_ons) {
			$session->checkout_add_ons = [
				'fields' => [],
				'fees'   => [],
				'files'  => [],
			];
	...
	public function checkout_get_add_on_value( $value, $input ) {
		...
		if ( isset( $this->wc_session()->checkout_add_ons['fields'][ $input ] ) ) {
			$value = $this->wc_session()->checkout_add_ons['fields'][ $input ]['value'];
		}
	...
	public function store_add_on_in_session( $id, $name, $value ) {
		...
		$session_data = $this->wc_session()->checkout_add_ons ?? [
			'fields' => [],
			'fees'   => [],
		];
		...
		$this->wc_session()->checkout_add_ons = $session_data;
	}
	...
	public function clear_session() {
		...
		unset( $this->wc_session()->checkout_add_ons );
	}
	...
	public function display_add_on_value_in_checkout_order_review( $safe_text, $text ) {
		...
		if ( isset( $this->wc_session()->checkout_add_ons['fees'][ $text ] ) ) {

			$session_data = $this->wc_session()->checkout_add_ons['fees'][ $text ];
	...
	public function save_add_on_meta( $item_id, $item, $order_id ) {
		...
		if ( isset( $this->wc_session()->checkout_add_ons['fees'][ $fee_id ] ) ) {

			$add_on_data = $this->wc_session()->checkout_add_ons['fees'][ $fee_id ];
	...
	/**
	 * Get the session handler and initialize it, it not.
	 *
	 * @return WC_Session
	 */
	protected function wc_session() : WC_Session
	{
		if (! WC()->session) {
