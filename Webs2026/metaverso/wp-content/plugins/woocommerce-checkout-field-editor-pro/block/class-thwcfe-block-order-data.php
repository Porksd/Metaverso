...
if(!class_exists('THWCFE_Block_Order_Data')):
 
class THWCFE_Block_Order_Data {
	...
	***********************************************/
		...
		if($type === 'label' || $type === 'heading' || $type === 'paragraph'){
			...
			if(!empty($value) || (($value === '0') && apply_filters( 'thwcfe_accept_value_zero',false))){
				...
				if($type === 'file'){
					...
					if($type === 'checkbox'){
						$value = ($value === 'yes' || $value === '1') ? __('Yes', 'woocommerce-checkout-field-editor-pro') : __('No', 'woocommerce-checkout-field-editor-pro');
					}
    ...
    private function get_single_field_html_admin_order($field){
		...
		if($type === 'heading' || $type === 'label' || $type === 'paragraph'){
			$label = isset($field['label']) ? THWCFE_i18n::t($field['label']) : false;
