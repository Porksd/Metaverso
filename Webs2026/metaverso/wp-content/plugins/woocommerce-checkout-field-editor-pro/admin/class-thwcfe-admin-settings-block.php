...
if(!class_exists('THWCFE_Admin_Settings_Block')):

class THWCFE_Admin_Settings_Block extends THWCFE_Admin_Settings {
	...
	public function init(){
		...
		$this->section_form = new THWCFE_Admin_Form_Block_Section();
		$this->field_form   = new THWCFE_Admin_Form_Block_Field();
	...
	private function reset_to_default() {
		...
		delete_option(THWCFE_Utils_Block::OPTION_KEY_BLOCK_SECTIONS);
	...
	private function reset_section($section_name){
		...
		if($currentSection && $section_name){
           ...
           $this->reset_block_section_fields($sections, $section_name, $currentSection);
		}
	...
	public function reset_block_section_fields($all_sections, $section_name, $sectionCopy){

		$checkout_fields = THWCFE_Utils_Block::get_default_block_fields();
		$default_fields = isset($checkout_fields[$section_name]) ? $checkout_fields[$section_name] : array();
		$sectionCopy->set_property('fields', THWCFE_Utils_Block::prepare_default_fields($default_fields));

		if(THWCFE_Utils_Section::is_valid_section($sectionCopy)){
			$all_sections[$section_name] = $sectionCopy;
			$this->save_block_sections($all_sections);
		}
	}
	...
	public function render_sections() {
		...
		foreach( $sections as $name => $section ){
			...
			$props_json = htmlspecialchars(THWCFE_Utils_Section::get_property_json($section, 'block'));
			...
			echo '<li><a href="'. $url .'" class="'. ($current_section == $name ? 'current' : '') .'">'. esc_html($title) .'</a></li>';
			if(THWCFE_Utils_Section::is_custom_section($section)){

				?>
                <li>
                	<form id="section_prop_form_<?php echo esc_attr($name); ?>" method="post" action="">
                        <input type="hidden" name="f_rules[<?php echo esc_attr($i); ?>]" class="f_rules" value="<?php echo $rules_json; ?>" />
                        <input type="hidden" name="f_rules_ajax[<?php echo esc_attr($i); ?>]" class="f_rules_ajax" value="<?php echo $rules_json_ajax; ?>" />
                    </form>
                    <span class="s_edit_btn dashicons dashicons-edit tips" 
      					data-tip="<?php esc_attr_e('Edit Section', 'woocommerce-checkout-field-editor-pro'); ?>" 
      					onclick="thwcfeOpenEditSectionForm(<?php echo esc_js($props_json); ?>, 'block')">
					</span>
                </li>
                <li>
					<span class="s_copy_btn dashicons dashicons-admin-page tips" 
						data-tip="<?php _e('Duplicate Section', 'woocommerce-checkout-field-editor-pro'); ?>" 
						onclick="thwcfeOpenCopySectionForm(<?php echo esc_js($props_json); ?>, 'block')">
					</span>
				</li>
				<li>
		...
		echo '<li><a href="javascript:thwcfeOpenNewSectionForm(\'block\')" class="btn btn-tiny btn-primary ml-30">+ '. __('Add new section', 'woocommerce-checkout-field-editor-pro') .'</a></li>';
		echo '</ul>';		
	...
	private function create_section(){
		...
		$section = THWCFE_Utils_Section::prepare_section_from_posted_data($_POST, 'new', 'block');
	...
	private function edit_section(){
		$result = false;
		$section  = THWCFE_Utils_Section::prepare_section_from_posted_data($_POST, 'edit', 'block');
	...
	private function delete_section($section_name){
		if(isset($section_name) && !empty($section_name)){	
			...
			if(is_array($sections) && isset($sections[$section_name])){
				...
				$result = $this->save_block_sections($sections);		
				return $result;
			}
		}
		return false;
	...
	private function render_fields_table_heading(){
		...
		<th class="name"><?php esc_html_e('Name', 'woocommerce-checkout-field-editor-pro'); ?></th>
		<th class="type"><?php esc_html_e('Type', 'woocommerce-checkout-field-editor-pro'); ?></th>
		<th class="label"><?php esc_html_e('Label', 'woocommerce-checkout-field-editor-pro'); ?></th>
		<th class="placeholder"><?php esc_html_e('Placeholder', 'woocommerce-checkout-field-editor-pro'); ?></th>
		<th class="validate"><?php esc_html_e('Validations', 'woocommerce-checkout-field-editor-pro'); ?></th>
        <th class="status"><?php esc_html_e('Required', 'woocommerce-checkout-field-editor-pro'); ?></th>
		<th class="status"><?php esc_html_e('Enabled', 'woocommerce-checkout-field-editor-pro'); ?></th>	
        <th class="actions align-center"><?php esc_html_e('Actions', 'woocommerce-checkout-field-editor-pro'); ?></th>	
        <?php
	}
	...
	private function render_actions_row($section){
		...
		?>
            <th colspan="5">
                <button type="button" class="btn btn-small btn-primary" onclick="thwcfeOpenNewFieldForm('<?php echo $section->get_property('name'); ?>', 'block')">
...
<!--                 <input type="submit" name="reset_fields" class="btn btn-small" value="<?php THWCFE_i18n::et('Reset to default fields') ?>" style="float:right; margin-right: 5px;" 
		...
		?>
			<th colspan="5">
                <button type="button" class="btn btn-small" disabled ><?php esc_html_e('+ Add field', 'woocommerce-checkout-field-editor-pro'); ?></button>
                <button type="button" class="btn btn-small" disabled ><?php esc_html_e('Remove', 'woocommerce-checkout-field-editor-pro'); ?></button>
                <button type="button" class="btn btn-small" disabled ><?php esc_html_e('Enable', 'woocommerce-checkout-field-editor-pro'); ?></button>
                <button type="button" class="btn btn-small" disabled ><?php esc_html_e('Disable', 'woocommerce-checkout-field-editor-pro'); ?></button>
            </th>
            <th colspan="5">
...
<!--                 <input type="submit" name="reset_fields" class="btn btn-small" value="<?php THWCFE_i18n::et('Reset to default fields') ?>" style="float:right; margin-right: 5px;" 
	...
	private function render_content(){
		...
		$thwcfe_all_section_data = get_option(THWCFE_Utils_Block::OPTION_KEY_BLOCK_SECTIONS);
		$reset_to_default = '';
        ...
        <div class="wrap woocommerce"><div class="icon32 icon32-attributes" id="icon-woocommerce"><br /></div>                
            ...
            <table id="thwcfe_checkout_fields" class="wc_gateways widefat thpladmin_fields_table" cellspacing="0">
				...
				if(THWCFE_Utils_Section::is_valid_section($section) && THWCFE_Utils_Section::has_fields($section)){
					...
					?>
						<tr class="row_<?php echo $i; echo($is_enabled === 1 ? '' : ' thpladmin-disabled');  echo esc_attr($non_editable_field); echo esc_attr($not_deletable_field)?>">
							...
							<td class="td_actions" align="center">
								...
								<?php }else{ ?>
									...
									onclick="thwcfeOpenEditFieldForm(this, <?php echo $i; ?>, 'block')"></span>
								<?php } ?>
								...
								<?php }else{ ?>
									...
									onclick="thwcfeOpenCopyFieldForm(this, <?php echo $i; ?>, 'block')"></span>
								<?php } ?>
	...
	public function save_block_sections($sections){
		...
		$result = update_option(THWCFE_Utils_Block::OPTION_KEY_BLOCK_SECTIONS , $sections, 'no');
		return $result;
	}
	/*-----------------------------------*
	 *------ FIELD FORMS - END ----------*
	...
	*******************************************************************************/
	...
	private function move_fields_from_one_to_another(){
		...
		if(is_array($fields) && THWCFE_Utils_Section::is_valid_section($section_from) && THWCFE_Utils_Section::is_valid_section($section_to)){
			...
			if($updated){
				$result = $this->update_block_section($section_to);
			}
		}
	}
