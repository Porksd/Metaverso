...
if(!class_exists('THWCFE_Admin_Form_Block_Field')):

class THWCFE_Admin_Form_Block_Field extends THWCFE_Admin_Form{
	...
	private function get_field_types(){
		return array(
			...
			// 'datepicker' => 'Date Picker (jquery ui)', 
			// 'datetime_local' => 'Datetime local',
			// 'date' => 'Date', 
			// 'timepicker' => 'Time Picker (jquery)', 
			// 'time'=>'Time',
			// 'month' => 'Month', 
			// 'week' => 'Week', 
			// 'file' => 'File Upload', 
	...
	public function get_field_form_props(){
		...
		return array(
			...
			'type' 		  => array('type'=>'select', 'name'=>'type', 'label'=>'Field Type', 'required'=>1, 'options'=>$field_types, 'onchange'=>"thwcfeFieldTypeChangeListner(this, 'block')"),
	...
	private function output_field_form_pp( $section = ''){
		?>
        <div id="thwcfe_field_form_pp" class="thpladmin-modal-mask thadmin-block-form <?php echo esc_attr($section)?>">
							...
							<aside>
								...
								<ul class="pp_nav_links">
									...
									if($section !== 'address'){ ?>
										...
										<li class="text-primary pp-nav-link-rules <?php echo esc_attr($section) ?>" data-index="2">
	...
	private function render_form_tab_general_info( $section = '' ){
		...
		<div style="display: inherit;" class="data-panel-content">
			...
			<table class="thwcfe_field_form_tab_general_field_options thwcfe_block_field_options" style="display: none;">
				<?php
					$this->render_form_fragment_options();
				?>
			</table>
	...
	*/
	...
	private function render_form_fragment_general($section = ''){
		...
		$ad_field_props_type =  array('type'=>'select', 'name'=>'type', 'label'=>'Field Type', 'required'=>1, 'options'=>$address_field_types, 'onchange'=>"thwcfeFieldTypeChangeListner(this, 'block')");
	...
	private function render_form_field_checkbox( $section = '' ){
        ...
        <table id="thwcfe_field_form_id_checkbox" class="thwcfe_field_form_table" width="100%" style="display:none;">
			...
			//$this->render_form_elm_row_cb($this->field_props['required']); // can add required from WC version 9.8
			$this->render_form_elm_row_cb($this->field_props['enabled']);
