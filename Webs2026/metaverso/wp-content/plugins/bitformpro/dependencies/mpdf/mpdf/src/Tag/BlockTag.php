...
use BitCode\BitFormPro\Dependencies\Mpdf\Conversion\DecToAlpha;
use BitCode\BitFormPro\Dependencies\Mpdf\Conversion\DecToRoman;
	...
	public function open($attr, &$ahtml, &$ihtml)
	{
		...
		if ((isset($p['POSITION'])
			...
			if ($this->mpdf->inFixedPosBlock) {
				throw new \BitCode\BitFormPro\Dependencies\Mpdf\MpdfException('Cannot nest block with position:fixed or position:absolute');
			}
		...
		if ($currblk['inner_width'] < $mw) {
			...
			// if ($currblk['inner_width'] < $mw) { throw new \BitCode\BitFormPro\Dependencies\Mpdf\MpdfException("DIV is too narrow for text to fit!"); }
		}
