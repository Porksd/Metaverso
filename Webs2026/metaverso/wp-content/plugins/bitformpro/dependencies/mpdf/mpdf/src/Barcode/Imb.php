 ...
 */
class Imb extends \BitCode\BitFormPro\Dependencies\Mpdf\Barcode\AbstractBarcode implements \BitCode\BitFormPro\Dependencies\Mpdf\Barcode\BarcodeInterface
{
	...
	public function __construct($code, $xDim, $gapWidth, $daft)
	{
		if (!function_exists('bcadd')) {
			throw new \BitCode\BitFormPro\Dependencies\Mpdf\Barcode\BarcodeException('IMB barcodes require bcmath extension to be loaded.');
		}
	...
	private function init($code, $gapWidth, $daft)
	{
		...
		switch (strlen($routingCode)) {
			...
			case 11:
				...
				break;
			default:
				throw new \BitCode\BitFormPro\Dependencies\Mpdf\Barcode\BarcodeException(sprintf('Invalid MSI routing code "%s"', $routingCode));
		}
