...
class CssManager
{
	...
	private function getFileContents($path)
	{
		...
		if ($wrapperChecker->hasBlacklistedStreamWrapper($path)) {
			throw new \BitCode\BitFormPro\Dependencies\Mpdf\MpdfException('File contains an invalid stream. Only ' . implode(', ', $wrapperChecker->getWhitelistedStreamWrappers()) . ' streams are allowed.');
		}
		...
		if ($this->mpdf->basepathIsLocal) {
			...
			} catch (\BitCode\BitFormPro\Dependencies\Mpdf\MpdfException $e) {
				// Ignore error
			}
