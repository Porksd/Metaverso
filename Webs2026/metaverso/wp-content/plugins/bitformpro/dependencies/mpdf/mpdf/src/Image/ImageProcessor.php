...
namespace BitCode\BitFormPro\Dependencies\Mpdf\Image;
...
use BitCode\BitFormPro\Dependencies\Mpdf\Gif\Gif;
use BitCode\BitFormPro\Dependencies\Mpdf\Language\LanguageToFontInterface;
use BitCode\BitFormPro\Dependencies\Mpdf\Language\ScriptToLanguageInterface;
...
use BitCode\BitFormPro\Dependencies\Mpdf\RemoteContentFetcher;
use BitCode\BitFormPro\Dependencies\Mpdf\SizeConverter;
use BitCode\BitFormPro\Dependencies\Psr\Log\LoggerInterface;

class ImageProcessor implements \BitCode\BitFormPro\Dependencies\Psr\Log\LoggerAwareInterface
{
	...
	/**
	 * @var \BitCode\BitFormPro\Dependencies\Mpdf\Image\ImageTypeGuesser
	 */
	private $guesser;
	...
	public function getImage(&$file, $firsttime = true, $allowvector = true, $orig_srcpath = false, $interpolation = false)
	{
		...
		if ($type === 'jpeg' || $type === 'jpg') {
			...
			if ($a[2] === 'DeviceCMYK' && ($this->mpdf->restrictColorSpace === 2 || ($this->mpdf->PDFA && $this->mpdf->restrictColorSpace !== 3))) {
				...
				if (!function_exists('gd_info')) {
					throw new \BitCode\BitFormPro\Dependencies\Mpdf\MpdfException(sprintf('JPG image may not use CMYK color space (%s).', $file));
				}
		...
		if ($type === 'png') {
			...
			if ($firsttime && ($colspace === 'DeviceRGB' || $colspace === 'Indexed') && ($this->mpdf->PDFX || $this->mpdf->restrictColorSpace === 3)) {
				...
				if ($pngalpha) {
					if ($this->mpdf->PDFA) {
						throw new \BitCode\BitFormPro\Dependencies\Mpdf\MpdfException(sprintf('PDFA1-b does not permit images with alpha channel transparency (%s).', $file));
					}
	...
	private function gzCompress($data)
	{
		if (!function_exists('gzcompress')) {
			throw new \BitCode\BitFormPro\Dependencies\Mpdf\MpdfException('gzcompress is not available. install ext-zlib extension.');
		}
	...
	private function imageError($file, $firsttime, $msg)
	{
		...
		if ($firsttime && ($this->mpdf->showImageErrors || $this->mpdf->debug)) {
			throw new \BitCode\BitFormPro\Dependencies\Mpdf\MpdfImageException(sprintf('%s (%s)', $msg, substr($file, 0, 256)));
		}
