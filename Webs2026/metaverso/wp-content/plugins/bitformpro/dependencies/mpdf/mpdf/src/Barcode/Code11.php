 ...
 */
class Code11 extends \BitCode\BitFormPro\Dependencies\Mpdf\Barcode\AbstractBarcode implements \BitCode\BitFormPro\Dependencies\Mpdf\Barcode\BarcodeInterface
{
	...
	private function init($code, $printRatio)
	{
		...
		for ($i = 0; $i < $len; ++$i) {
			...
			if (!isset($chr[$code[$i]])) {
				throw new \BitCode\BitFormPro\Dependencies\Mpdf\Barcode\BarcodeException(sprintf('Invalid character "%s" in CODE11 barcode value "%s"', $code[$i], $code));
			}
