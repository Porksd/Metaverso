...
use BitCode\BitFormPro\Dependencies\Mpdf\Config\FontVariables;
use BitCode\BitFormPro\Dependencies\Mpdf\Conversion;
use BitCode\BitFormPro\Dependencies\Mpdf\Css\Border;
use BitCode\BitFormPro\Dependencies\Mpdf\Css\TextVars;
use BitCode\BitFormPro\Dependencies\Mpdf\Log\Context as LogContext;
use BitCode\BitFormPro\Dependencies\Mpdf\Fonts\MetricsGenerator;
use BitCode\BitFormPro\Dependencies\Mpdf\Output\Destination;
use BitCode\BitFormPro\Dependencies\Mpdf\QrCode;
use BitCode\BitFormPro\Dependencies\Mpdf\Utils\Arrays;
use BitCode\BitFormPro\Dependencies\Mpdf\Utils\NumericString;
use BitCode\BitFormPro\Dependencies\Mpdf\Utils\UtfString;
use BitCode\BitFormPro\Dependencies\Psr\Log\LoggerInterface;
use BitCode\BitFormPro\Dependencies\Psr\Log\NullLogger;
 ...
 * @license GPL-2.0
 */
class Mpdf implements \BitCode\BitFormPro\Dependencies\Psr\Log\LoggerAwareInterface
{
	...
	/**
	 * @var \BitCode\BitFormPro\Dependencies\Mpdf\Gradient
	 */
	private $gradient;

	/**
	 * @var \BitCode\BitFormPro\Dependencies\Mpdf\Image\Bmp
	 */
	private $bmp;

	/**
	 * @var \BitCode\BitFormPro\Dependencies\Mpdf\Image\Wmf
	 */
	private $wmf;
	...
	/**
	 * @var \BitCode\BitFormPro\Dependencies\Mpdf\DirectWrite
	 */
	private $directWrite;
	...
	/**
	 * @var \BitCode\BitFormPro\Dependencies\Mpdf\Fonts\FontCache
	 */
	private $fontCache;

	/**
	 * @var \BitCode\BitFormPro\Dependencies\Mpdf\Fonts\FontFileFinder
	 */
	private $fontFileFinder;

	/**
	 * @var \BitCode\BitFormPro\Dependencies\Mpdf\Tag
	 */
	private $tag;

	/**
	 * @var \BitCode\BitFormPro\Dependencies\Mpdf\Barcode
	 * @todo solve Tag dependency and make private
	 */
	public $barcode;

	/**
	 * @var \BitCode\BitFormPro\Dependencies\Mpdf\QrCode\QrCode
	 */
	private $qrcode;
	...
	/**
	 * @var \BitCode\BitFormPro\Dependencies\Mpdf\Color\ColorModeConverter
	 */
	private $colorModeConverter;

	/**
	 * @var \BitCode\BitFormPro\Dependencies\Mpdf\Color\ColorSpaceRestrictor
	 */
	private $colorSpaceRestrictor;

	/**
	 * @var \BitCode\BitFormPro\Dependencies\Mpdf\Hyphenator
	 */
	private $hyphenator;

	/**
	 * @var \BitCode\BitFormPro\Dependencies\Mpdf\Pdf\Protection
	 */
	private $protection;

	/**
	 * @var \BitCode\BitFormPro\Dependencies\Mpdf\RemoteContentFetcher
	 */
	...
	/**
	 * @var \BitCode\BitFormPro\Dependencies\Mpdf\Language\ScriptToLanguageInterface
	 */
	...
	/**
	 * @var \BitCode\BitFormPro\Dependencies\Mpdf\Writer\FontWriter
	 */
	private $fontWriter;

	/**
	 * @var \BitCode\BitFormPro\Dependencies\Mpdf\Writer\MetadataWriter
	 */
	private $metadataWriter;

	/**
	 * @var \BitCode\BitFormPro\Dependencies\Mpdf\Writer\ImageWriter
	 */
	private $imageWriter;

	/**
	 * @var \BitCode\BitFormPro\Dependencies\Mpdf\Writer\FormWriter
	 */
	private $formWriter;

	/**
	 * @var \BitCode\BitFormPro\Dependencies\Mpdf\Writer\PageWriter
	 */
	private $pageWriter;

	/**
	 * @var \BitCode\BitFormPro\Dependencies\Mpdf\Writer\BookmarkWriter
	 */
	private $bookmarkWriter;

	/**
	 * @var \BitCode\BitFormPro\Dependencies\Mpdf\Writer\OptionalContentWriter
	 */
	private $optionalContentWriter;

	/**
	 * @var \BitCode\BitFormPro\Dependencies\Mpdf\Writer\ColorWriter
	 */
	private $colorWriter;

	/**
	 * @var \BitCode\BitFormPro\Dependencies\Mpdf\Writer\BackgroundWriter
	 */
	private $backgroundWriter;

	/**
	 * @var \BitCode\BitFormPro\Dependencies\Mpdf\Writer\JavaScriptWriter
	 */
	private $javaScriptWriter;

	/**
	 * @var \BitCode\BitFormPro\Dependencies\Mpdf\Writer\ResourceWriter
	 */
	...
	public function __construct(array $config = [])
	{
		...
		if (file_exists($this->defaultCssFile)) {
			...
			throw new \BitCode\BitFormPro\Dependencies\Mpdf\MpdfException(sprintf('Unable to read default CSS file "%s"', $this->defaultCssFile));
		}
	...
	/**
	 * @param \BitCode\BitFormPro\Dependencies\Psr\Log\LoggerInterface
	 *
	 * @return \BitCode\BitFormPro\Dependencies\Mpdf\Mpdf
	 */
	...
	function _setPageSize($format, &$orientation)
	{
		if (is_string($format)) {
			...
			if (!$format[0] || !$format[1]) {
				throw new \BitCode\BitFormPro\Dependencies\Mpdf\MpdfException('Invalid page format: ' . $format[0] . ' ' . $format[1]);
			}
		...
		if ($orientation === 'p' || $orientation == 'portrait') {
			...
			throw new \BitCode\BitFormPro\Dependencies\Mpdf\MpdfException('Incorrect orientation: ' . $orientation);
		}
	...
	function SetDisplayMode($zoom, $layout = 'continuous')
	{
		...
		if (in_array($zoom, $allowedZoomModes, true) || is_numeric($zoom)) {
			...
			throw new \BitCode\BitFormPro\Dependencies\Mpdf\MpdfException('Incorrect zoom display mode: ' . $zoom);
		}
		...
		if (in_array($layout, $allowedLayoutModes, true)) {
			...
			throw new \BitCode\BitFormPro\Dependencies\Mpdf\MpdfException('Incorrect layout display mode: ' . $layout);
		}
	}
	...
	function SetVisibility($v)
	{
		...
		if ($v == 'printonly') {
			...
			throw new \BitCode\BitFormPro\Dependencies\Mpdf\MpdfException('Incorrect visibility: ' . $v);
		}
		$this->visibility = $v;
	}
	...
	function AddSpotColorsFromFile($file)
	{
		$colors = @file($file);
		if (!$colors) {
			throw new \BitCode\BitFormPro\Dependencies\Mpdf\MpdfException("Cannot load spot colors file - " . $file);
		}
	...
	function AddFont($family, $style = '')
	{
		...
		if ($this->usingCoreFont) {
			throw new \BitCode\BitFormPro\Dependencies\Mpdf\MpdfException("mPDF Error - problem with Font management");
		}
		...
		if (!isset($this->fontdata[$family][$stylekey]) || !$this->fontdata[$family][$stylekey]) {
			throw new \BitCode\BitFormPro\Dependencies\Mpdf\MpdfException(sprintf('Font "%s%s%s" is not supported', $family, $style ? ' - ' : '', $style));
		}
	...
	function SetFont($family, $style = '', $size = 0, $write = true, $forcewrite = false)
	{
		...
		if ($family == '') {
			if ($this->FontFamily) {
				...
				throw new \BitCode\BitFormPro\Dependencies\Mpdf\MpdfException("No font or default font set!");
			}
		}
		...
		if (($family == 'csymbol') || ($family == 'czapfdingbats') || ($family == 'ctimes') || ($family == 'ccourier') || ($family == 'chelvetica')) {
			if ($this->PDFA || $this->PDFX) {
				if ($family == 'csymbol' || $family == 'czapfdingbats') {
					throw new \BitCode\BitFormPro\Dependencies\Mpdf\MpdfException("Symbol and Zapfdingbats cannot be embedded in mPDF (required for PDFA1-b or PDFX/1-a).");
				}
		...
		if (!$this->onlyCoreFonts && !$this->usingCoreFont) {
			...
			if ($this->PDFA || $this->PDFX) {
				throw new \BitCode\BitFormPro\Dependencies\Mpdf\MpdfException('Core Adobe fonts cannot be embedded in mPDF (required for PDFA1-b or PDFX/1-a) - cannot use option to use core fonts.');
			}
			...
			if (!isset($this->fonts[$fontkey])) {
				...
				if (isset($this->CoreFonts[$fontkey])) {
					...
					if (!isset($cw)) {
						throw new \BitCode\BitFormPro\Dependencies\Mpdf\MpdfException(sprintf('Could not include font metric file "%s"', $file));
					}
					$i = count($this->fonts) + $this->extraFontSubsets + 1;
					...
					throw new \BitCode\BitFormPro\Dependencies\Mpdf\MpdfException(sprintf('Font %s not defined', $fontkey));
				}
			}
		...
		foreach ($this->objectbuffer as $ib => $objattr) {
			...
			if ($objattr['type'] === 'barcode') {
				...
				if (in_array($objattr['btype'], $barcodeTypes, true)) {
					...
					if (!class_exists('BitCode\BitFormPro\Dependencies\Mpdf\QrCode\QrCode') || !class_exists('BitCode\BitFormPro\Dependencies\Mpdf\QrCode\Output\Mpdf')) {
						throw new \BitCode\BitFormPro\Dependencies\Mpdf\MpdfException('BitCode\BitFormPro\Dependencies\Mpdf\QrCode package was not found. Install the package from Packagist with "composer require mpdf/qrcode"');
					}
	...
	function _SetTextRendering($mode)
	{
		if (!(($mode == 0) || ($mode == 1) || ($mode == 2))) {
			throw new \BitCode\BitFormPro\Dependencies\Mpdf\MpdfException("Text rendering mode should be 0, 1 or 2 (value : $mode)");
		}
		$tr = ($mode . ' Tr');
	...
	function Output($name = '', $dest = '')
	{
		...
		if ($this->debug && error_get_last()) {
			...
			if (($e['type'] < 2048 && $e['type'] != 8) || (intval($e['type']) & intval(ini_get("error_reporting")))) {
				throw new \BitCode\BitFormPro\Dependencies\Mpdf\MpdfException(
		...
		if (($this->PDFA || $this->PDFX) && $this->encrypted) {
			throw new \BitCode\BitFormPro\Dependencies\Mpdf\MpdfException('PDF/A1-b or PDF/X1-a does not permit encryption of documents.');
		}
		...
		if (count($this->PDFAXwarnings) && (($this->PDFA && !$this->PDFAauto) || ($this->PDFX && !$this->PDFXauto))) {
			...
			throw new \BitCode\BitFormPro\Dependencies\Mpdf\MpdfException('PDFA/PDFX warnings generated. See log for further details');
		}
		...
		switch ($dest) {
			...
			case Destination::INLINE:
				...
				if ($this->debug && !$this->allow_output_buffering && ob_get_contents()) {
					throw new \BitCode\BitFormPro\Dependencies\Mpdf\MpdfException('Output has already been sent from the script - PDF file generation aborted.');
				}
			...
			case Destination::DOWNLOAD:
				...
				if (headers_sent()) {
					throw new \BitCode\BitFormPro\Dependencies\Mpdf\MpdfException('Data has already been sent to output, unable to output PDF file');
				}
			...
			case Destination::FILE:
				...
				if (!$f) {
					throw new \BitCode\BitFormPro\Dependencies\Mpdf\MpdfException(sprintf('Unable to create output file %s', $name));
				}
			...
			default:
				throw new \BitCode\BitFormPro\Dependencies\Mpdf\MpdfException(sprintf('Incorrect output destination %s', $dest));
		}
	...
	function _dochecks()
	{
		...
		if (1.1 == 1) {
			throw new \BitCode\BitFormPro\Dependencies\Mpdf\MpdfException('Do not alter the locale before including mPDF');
		}
		...
		if (ini_get('mbstring.func_overload')) {
			throw new \BitCode\BitFormPro\Dependencies\Mpdf\MpdfException('Mpdf cannot function properly with mbstring.func_overload enabled');
		}

		if (!function_exists('mb_substr')) {
			throw new \BitCode\BitFormPro\Dependencies\Mpdf\MpdfException('mbstring extension must be loaded in order to run mPDF');
		}
		...
		if (!function_exists('mb_regex_encoding')) {
			...
			throw new \BitCode\BitFormPro\Dependencies\Mpdf\MpdfException('mbstring extension with mbregex support must be loaded in order to run mPDF.' . $mamp);
		}
	}
	...
	{
		if ($this->PDFA || $this->PDFX) {
			throw new \BitCode\BitFormPro\Dependencies\Mpdf\MpdfException('PDFA and PDFX do not permit transparency, so mPDF does not allow Watermarks!');
		}
	...
	function AddCIDFont($family, $style, $name, &$cw, $CMap, $registry, $desc)
	{
		...
		if (isset($this->fonts[$fontkey])) {
			throw new \BitCode\BitFormPro\Dependencies\Mpdf\MpdfException("Font already added: $family $style");
		}
		$i = count($this->fonts) + $this->extraFontSubsets + 1;
	...
	function AddCJKFont($family)
	{
		...
		if ($this->PDFA || $this->PDFX) {
			throw new \BitCode\BitFormPro\Dependencies\Mpdf\MpdfException("Adobe CJK fonts cannot be embedded in mPDF (required for PDFA1-b and PDFX/1-a).");
		}
		if ($family == 'big5') {
			$this->AddBig5Font();
		} elseif ($family == 'gb') {
	...
	function TableHeaderFooter($content = '', $tablestartpage = '', $tablestartcolumn = '', $horf = 'H', $level = 0, $firstSpread = true, $finalSpread = true)
	{
		if (($horf == 'H' || $horf == 'F') && !empty($content)) { // mPDF 5.7.2
			...
			if (empty($content[$firstrow])) {
				if ($this->debug) {
					throw new \BitCode\BitFormPro\Dependencies\Mpdf\MpdfException("<tfoot> must precede <tbody> in a table");
				} else {
					return;
				}
			}
	...
	function WriteHTML($html, $mode = HTMLParserMode::DEFAULT_MODE, $init = true, $close = true)
	{
		...
		if (is_scalar($html) === false) {
			if (!is_object($html) || ! method_exists($html, '__toString')) {
				throw new \BitCode\BitFormPro\Dependencies\Mpdf\MpdfException('WriteHTML() requires $html be an integer, float, string, boolean or an object with the __toString() magic method.');
			}
		}

		// Check the mode is valid
		if (in_array($mode, HTMLParserMode::getAllModes(), true) === false) {
			throw new \BitCode\BitFormPro\Dependencies\Mpdf\MpdfException('WriteHTML() requires $mode to be one of the modes defined in HTMLParserMode');
		}
	...
	function printbuffer($arrayaux, $blockstate = 0, $is_table = false, $table_draft = false, $cell_dir = '')
	{
		...
		for ($i = 0; $i < $array_size; $i++) {
			...
			if (($blockdir == 'rtl' || $this->biDirectional) && isset($arrayaux[$i][4]) && in_array($arrayaux[$i][4], ['ccourier', 'ctimes', 'chelvetica', 'csymbol', 'czapfdingbats'])) {
				throw new \BitCode\BitFormPro\Dependencies\Mpdf\MpdfException("You cannot use core fonts in a document which contains RTL text.");
			}
		}
		// mPDF 6
	...
	// mPDF 6
		...
		if (($toonarrow) && ($this->table_error_report)) {
			throw new \BitCode\BitFormPro\Dependencies\Mpdf\MpdfException("Word is too long to fit in table - " . $this->table_error_report_param);
		}
		if ($toonarrow) {
			return -$biggestword;
		} else {
		...
		if (!$this->is_utf8($html)) {
			...
			while (mb_convert_encoding(mb_convert_encoding($html, "UTF-32", "UTF-8"), "UTF-8", "UTF-32") != $html) {
				...
				if ($error && $error['message'] === 'iconv(): Detected an illegal character in input string') {
					throw new \BitCode\BitFormPro\Dependencies\Mpdf\MpdfException('Invalid input characters. Did you set $mpdf->in_charset properly?');
				}
			...
			throw new \BitCode\BitFormPro\Dependencies\Mpdf\MpdfException("HTML contains invalid UTF-8 character(s). See log for further details");
		}
	...
	function purify_utf8_text($txt)
	{
		...
		if (!$this->is_utf8($txt)) {
			throw new \BitCode\BitFormPro\Dependencies\Mpdf\MpdfException("Text contains invalid UTF-8 character(s)");
		}
	...
	function WriteBarcode($code, $showtext = 1, $x = '', $y = '', $size = 1, $border = 0, $paddingL = 1, $paddingR = 1, $paddingT = 2, $paddingB = 2, $height = 1, $bgcol = false, $col = false, $btype = 'ISBN', $supplement = '0', $supplement_code = '', $k = 1)
	{
		...
		if ($arrcode === false) {
			throw new \BitCode\BitFormPro\Dependencies\Mpdf\MpdfException('Error in barcode string: ' . $codestr);
		}
		...
		if ($supplement) { // EAN-2 or -5 Supplement
			...
			if ($supparrcode === false) {
				throw new \BitCode\BitFormPro\Dependencies\Mpdf\MpdfException('Error in barcode string (supplement): ' . $codestr . ' ' . $supplement_code);
			}

			if (strlen($supplement_code) != $supplement) {
				throw new \BitCode\BitFormPro\Dependencies\Mpdf\MpdfException('Barcode supplement incorrect: ' . $supplement_code);
			}
	...
	function transformScale($s_x, $s_y, $x = '', $y = '', $returnstring = false)
	{
		...
		if (($s_x == 0) or ( $s_y == 0)) {
			throw new \BitCode\BitFormPro\Dependencies\Mpdf\MpdfException('Please do not use values equal to zero for scaling');
		}
	...
	function AdjustHTML($html, $tabSpaces = 8)
	{
		...
		if (0 >= (int) $limit) {
			throw new \BitCode\BitFormPro\Dependencies\Mpdf\MpdfException(sprintf(
	...
	function OverWrite($file_in, $search, $replacement, $dest = Destination::DOWNLOAD, $file_out = "mpdf")
	{
		...
		switch ($dest) {
			...
			case Destination::FILE:
				...
				if (!$f) {
					throw new \BitCode\BitFormPro\Dependencies\Mpdf\MpdfException('Unable to create output file: ' . $file_out);
				}
