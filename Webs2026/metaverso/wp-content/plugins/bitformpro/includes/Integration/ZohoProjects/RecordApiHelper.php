...
class RecordApiHelper
{
	...
	private function createProject($dataCenter, $portalId, $data)
	{
		...
		// var_dump($createProjectEndpoint);
	...
	public function executeRecordApi($formID, $entryID, $projectsConf, $dataCenter, $fieldMap, $fieldValues)
	{
		...
		foreach ($subEvent as $sEvent) {
			...
			foreach ($fieldMap->{$sEvent} as $fieldKey => $fieldPair) {
				...
				if (empty($fieldData[$sEvent][$fieldPair->zohoFormField]) && \in_array($fieldPair->zohoFormField, $required)) {
					$error = new WP_Error('REQ_FIELD_EMPTY', wp_sprintf(__('%s is required for %s', 'bitformpro'), $fieldPair->zohoFormField, $sEvent));
					$this->_logResponse->apiResponse($this->_logID, $this->_integrationID, ['type' => 'record', 'type_name' => 'field'], 'validation', $error);
					return $error;
				}
			}

			if (isset($fieldData[$sEvent]['custom_fields'])) $fieldData[$sEvent]['custom_fields'] = json_encode($fieldData[$sEvent]['custom_fields']);

			// Actions Mapping
			foreach ($actions->{$sEvent} as $fieldKey => $action) {
				if (in_array($fieldKey, ['tags', 'attachments', 'users', 'timelog', 'bug_followers'])) continue;
				if ((gettype($action) === 'object' && count((array) $action)) || (gettype($action) === 'string' && !empty($action))) {
					if ($fieldKey === 'owner') {
						...
						} else if ($sEvent === 'issue') {
							$fieldData[$sEvent]['assignee'] = $action;
						} else {
							$fieldData[$sEvent]['owner'] = $action;
						}
					} else if ($fieldKey === 'reminder_string' && !empty($action->reminder_criteria)) {
						...
						if (isset($action->custom_date)) $action->custom_date = date_format(date_create($action->custom_date), "m-d-Y");
						if (isset($action->reminder_time)) $action->reminder_time = date("H:i", strtotime($action->reminder_time));

						unset($action->custom_date_fld, $action->reminder_time_fld);
						$fieldData[$sEvent][$fieldKey] = '{"reminder":[' . wp_json_encode($action) . ']}';
					} else if ($fieldKey === 'recurrence_string' && !empty($action->recurring_frequency)) {
						...
						if (!isset($action->time_span)) $action->time_span = "1";
						if (!isset($action->number_of_occurrences)) $action->number_of_occurrences = "2";
						if (!isset($action->is_comments_recurred)) $action->is_comments_recurred = 'false';
						if (!isset($action->set_previous_business_day)) $action->set_previous_business_day = 'false';
						if (!isset($action->recurrence_type)) $action->recurrence_type = 'specified_interval_creation';
		...
		if (in_array('tasklist', $subEvent)) {
			...
			} else if (array_key_exists('milestone', $eventApiResponse)) {
		...
		if (in_array('task', $subEvent)) {
			...
			} else if (array_key_exists('tasklist', $eventApiResponse)) {
		...
		foreach ($eventApiResponse as $eventKey => $eventId) {
			...
			if (isset($actions->{$eventKey}->timelog)) {
				...
				if (!isset($timelog->hours)) $timelog->hours = "0";
