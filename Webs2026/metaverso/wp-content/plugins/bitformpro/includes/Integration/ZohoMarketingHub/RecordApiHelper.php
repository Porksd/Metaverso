...
namespace BitCode\BitFormPro\Integration\ZohoMarketingHub;
    ...
    public function executeRecordApi($list, $dataCenter, $fieldValues, $fieldMap, $required)
    {
        ...
        foreach ($fieldMap as $fieldPair) {
            ...
            if (empty($fieldData[$fieldPair->zohoFormField]) && \in_array($fieldPair->zohoFormField, $required)) {
                $error = new WP_Error('REQ_FIELD_EMPTY', wp_sprintf(__('%s is required for zoho marketing hub', 'bitformpro'), $fieldPair->zohoFormField));
