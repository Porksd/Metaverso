...
var WP_Optimize_Premium = function() {
	...
	function show_images_loaded_text(blog_id) {
		if (images_total_count.hasOwnProperty(blog_id) && images_total_count[blog_id] > 0) {
			var text = wpoptimize.x_of_x_images_loaded.replace('%1$s', images_loaded_count[blog_id]).replace('%2$s', images_total_count[blog_id]);
	...
	function show_images_trash_loaded_text(blog_id) {
		if (images_trash_total_count.hasOwnProperty(blog_id)) {
			var text = wpoptimize.x_of_x_images_loaded.replace('%1$s', images_trash_loaded_count[blog_id]).replace('%2$s', images_trash_total_count[blog_id]);
	 ...
	 *
	 * @param {Object} data - metadata returned from images optimization
	 *
	 * @return void
	 */
	...
	function open_preview_dialog(data) {
		...
		dialog_buttons[wpoptimize.delete_selected_items_btn] = function() {
			...
			wp_optimize.send_command('do_optimization',
				...
				function (response) {
					...
					$('#wpo-preview-tablesorter')[0].dispatchEvent(new CustomEvent("reload", {}));
		...
		dialog.dialog({
			...
			modal: true,
			open: function() {
				$(this).dialog("option", "minHeight", 300); // Explicitly set minHeight
			},
			close: function() {
		 ...
		 */
		var ds = new WPO_Table_DataSource({
			optimization_id: optimization_id,
			limit: 1
		});
		...
		ds.fetch().done(
			function(response) {
				...
				table.append(['<thead><tr>',header.join(''),'</tr></thead>'].join(''));
				table.append(['<tfoot><tr>',footer.join(''),'</tr></tfoot>'].join(''));
				table.append('<tbody></tbody>');

				// initialize table sorter for displayed data.
				const tableEl = table[0];

				const table_pager = new WPO_TablePager({
					table: tableEl,
					container: document.getElementById('pager'),
					dataSource: ds
				});

				table_pager.loadTableData();

				// handle reload event, used when elements are deleting.
				tableEl.addEventListener('reload', function() {
					table_pager.loadTableData();
				});

				// handle loading start for preview.
				tableEl.addEventListener('load_start', function() {
					preview_loader.show();
				});

				// handle loading end data for preview.
				tableEl.addEventListener('load_end', function(e) {
	...
	/**
	 * Save DB analysis options.
	 *
	 * @param {function} callback Next function after successful save
	 * @return void
	 */
	 function save_table_analysis_options(callback) {
		var options = {};
		options['is_enabled'] = $('#enable-db-analysis').is(":checked");

		WP_Optimize.send_command('save_table_analysis_options', options, function(resp) {
			if (resp.message) {
				table_analysis_show_response_message(resp.message);
				callback();
			}
		});
	}
	...
	/**
	 * Hide the operation result message after 3 seconds
	 */
	function table_analysis_hide_response_message() {
		setTimeout(function() {
			$("#wpo-save-options-response").html('');
			$("#wpo-save-options-response").hide();
		}, 3000);
	}
	 ...
	 *
	 * @param {string} msg Response from backend
	 */
	function table_analysis_show_response_message(msg) {
		$("#wpo-save-options-response").show();
		$("#wpo-save-options-response").html(msg);

		table_analysis_hide_response_message();
	}

	/**
	 * Reload tab contents via AJAX
	 */
	function reload_table_analysis_report() {
		$("#wpo_loading_spinner span, #wpo_loading_spinner img").removeClass('hidden').show();

		if (is_table_analysis_enabled_and_loading() && !$('div.blockUI').is(':visible')) {
			wp_optimize.block_ui(wpoptimize.no_information_available_yet + ' ' +  wpoptimize.please_wait_a_few_seconds_while_we_refresh, {}, 0, false);
		}

		WP_Optimize.send_command('get_table_analysis_data', {"selected-plugin": $("#wpo-plugin-usage-select").val()}, function(resp) {
			$("#wpo_loading_spinner span, #wpo_loading_spinner img").addClass('hidden').hide();

			$('#wpo-table-analysis-report-container').html(resp.html);
			
			table_analysis_add_report_event_listeners();

			// If the data is still not available then we re-run the request.
			if (is_table_analysis_enabled_and_loading()) {
				setTimeout(reload_table_analysis_report, 5000);
			} else {
				$.unblockUI();
			}
	...
	/**
	 * Attach to settings elements events
	 */
	function table_analysis_add_settings_event_listeners() {
		$('#wpo-wipe-table-usage-data').on("click", function(evt) {
			evt.preventDefault();

			$('#wpo-save-options-response').html(wpoptimize.please_wait);

			WP_Optimize.send_command('wipe_table_analysis_data', {}, function(resp) {
				table_analysis_show_response_message(resp.message);

				reload_table_analysis_report();
			});
		});

		$('#enable-db-analysis').on("change", function() {
			save_table_analysis_options(reload_table_analysis_report);
		});
	}

	/**
	 * Verifies whether table analysis is enabled and data still not loaded.
	 *
	 * @returns bool
	 */
	function is_table_analysis_enabled_and_loading() {
		var is_enabled = $('#enable-db-analysis').prop('checked'),
			is_plugin_select_visible = $('#wpo-plugin-usage-select').is(':visible');

		return is_enabled && !is_plugin_select_visible;
	}

	/**
	 * Attach to report elements events
	 */
	function table_analysis_add_report_event_listeners() {
		$('#wpo-plugin-usage-select').on("change", function() {
			$('.wpo-plugin-tables-usage').hide();
			$('#wpo-plugin-tables-usage-' + this.value).show();

			if ('' != this.value) {
				$('#wpo-db-analysis-table-search, #wpo-table-analysis-reload').removeClass('hidden');
			} else {
				$('#wpo-db-analysis-table-search, #wpo-table-analysis-reload').addClass('hidden');
			}
		});

		$('#wpo-db-analysis-table-search').on('keyup', function() {
			var search_string = this.value;

			$('.wpo-plugin-tables-usage #the-list tr').each(function() {
				var the_row = this;
				$(this).children('td:nth-child(2)').each(function() {
					if (this.innerHTML.indexOf(search_string) > -1) {
						$(the_row).show();
					} else {
						$(the_row).hide();
					}
		...
		$('#wpo-db-analysis-table-search').on('keypress', function(event) {
			if (13 === event.keyCode) {
				event.preventDefault();
			}
		});

		$("#wpo-table-analysis-reload").on("click", reload_table_analysis_report);
	}

	table_analysis_add_settings_event_listeners();
	table_analysis_add_report_event_listeners();
	...
	// Export reload function for WP_Optimize_Premium()
	return {
		reload_table_analysis_report: reload_table_analysis_report
	}
