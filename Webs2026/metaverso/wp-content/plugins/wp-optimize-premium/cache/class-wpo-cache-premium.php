...
class WPO_Cache_Premium {
	...
	public function __construct() {
		...
		add_action('wpo_cache_config_updated', array($this, 'update_core_show_avatars'), 10, 1);
		add_action('update_option_show_avatars', array($this, 'update_gravatar_show_avatars'), 10, 2);
		
		if ($this->is_local_gravatars_enabled()) {
			$this->host_gravatars_locally();
		}
	}
	...
	public function always_purge_post_on_update($post_id) {
		...
		if ((defined('DOING_AUTOSAVE') && DOING_AUTOSAVE) || 'revision' === $post_type || is_null($post_type_object) || !$post_type_object->public) {
			return;
		}
		...
		if ($is_cache_purged) {
			WPO_Page_Cache::instance()->file_log("Cache for always_purge_post posts associated with post Title: {{title}} have been purged, triggered by action: " . current_filter(), $post_id);
		}
	}
	...
	public function add_default_values_to_cache_config($config) {
		...
		if (defined('WC_ABSPATH') && function_exists('WC')) {
			...
			if (isset($config['enable_cache_per_country']) && $config['enable_cache_per_country']) {
				...
				// We need the database and WooCommerce 3.4+ to use Geolite2
				$config['default_values']['wc_geolocate'] = isset($database) && file_exists($database) && version_compare(WC()->version, '3.4.0', '>=');
	...
	public function add_cache_control_metabox() {
		if (WPO_Page_Cache::instance()->is_enabled()) {
	...
	/**
	 * Schedule or clear cron jobs based on feature toggles.
	 *
	 * This includes:
	 * - Caching gravatars locally: schedules purging of cached gravatars and pruning logs.
	 * - Lazy load image preview cache: schedules deletion of YouTube preview image cache.
	 *
	 * The jobs are only scheduled if their respective features are enabled via options.
	 */
	private function cron_activate(): void {
		if ($this->is_local_gravatars_enabled()) {
			if (!wp_next_scheduled('wpo_purge_cached_gravatars')) {
				wp_schedule_event(time(), 'wpo_monthly', 'wpo_purge_cached_gravatars');
			}
			if (!wp_next_scheduled('wpo_prune_gravatar_logs')) {
				wp_schedule_event(time(), 'daily', 'wpo_prune_gravatar_logs');
			}
		} else {
			wp_clear_scheduled_hook('wpo_purge_cached_gravatars');
			wp_clear_scheduled_hook('wpo_prune_gravatar_logs');
		}

		if ($this->is_youtube_preview_enabled()) {
			if (!wp_next_scheduled('wpo_delete_lazyload_image_cache')) {
				wp_schedule_event(time() + 30 * 86400, 'wpo_monthly', 'wpo_delete_lazyload_image_cache');
			}
		} else {
			wp_clear_scheduled_hook('wpo_delete_lazyload_image_cache');
		}
	}
	
	/**
	 * Update WP Core show_avatars option.
	 *
	 * @param array $config
	 */
	public function update_core_show_avatars(array $config): void {
		update_option('show_avatars', (bool) $config['show_avatars']);
	}
	
	/**
	 * Update Gravatar `show_avatars` option based on core option
	 *
	 * @param boolean $old_value
	 * @param boolean $value
	 */
	public function update_gravatar_show_avatars(bool $old_value, ?bool $value): void {
		$this->config['show_avatars'] = (bool) $value;
		WPO_Cache_Config::instance()->update($this->config);
	}
	
	/**
	 * Instantiates Gravatar class to host gravatars locally
	 */
	private function host_gravatars_locally(): void {
		WPO_Gravatar::instance();
	}
	
	/**
	 * Determine if local gravatars are enabled or not
	 *
	 * @return bool
	 */
	private function is_local_gravatars_enabled(): bool {
		return (bool) $this->config['host_gravatars_locally'];
	}
	
	/**
	 * Determines if youtube preview is enabled or not
	 *
	 * @return bool
	 */
	private function is_youtube_preview_enabled(): bool {
		return (bool) WP_Optimize_Lazy_Load::instance()->get_lazy_options()['youtube_preview'];
	}
}
endif;
