...
class WP_Optimize_Premium {
	...
	public function wpo_js_translations($translations) {
		$premium_translations = array(
			...
			'no_information_available_yet' => __('No information available yet.', 'wp-optimize'),
			'please_wait_a_few_seconds_while_we_refresh' => __('Please wait a few seconds while we refresh...', 'wp-optimize')
		);
	...
	public function add_forgot_password_link($html, $slug) {
		if (isset($slug) && is_string($slug) && 'wp-optimize-premium' === $slug) {
			$html .= ' <a href="https://teamupdraft.com/my-account/lost-password/" target="_blank">' . esc_html__('Lost your password?', 'wp-optimize') . '</a>';
		}
