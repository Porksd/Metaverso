...
<div class="wpo-fieldgroup cache-options">
	<div class="wpo-fieldgroup__subgroup">
		<label for="wpo-show-avatars">
			<input name="show_avatars" id="wpo-show-avatars" class="cache-settings" type="checkbox" value="true" <?php checked(get_option('show_avatars'), 1); ?>>
	...
	<div class="wpo-fieldgroup__subgroup" id="wpo-host-gravatars-locally-container" style="<?php echo get_option('show_avatars') ? '' : 'display:none;'; ?>">
		<label for="wpo-host-gravatars-locally">
			<input name="host_gravatars_locally" id="wpo-host-gravatars-locally" class="cache-settings" type="checkbox" value="true" <?php checked($wpo_cache_options['host_gravatars_locally'], 1); ?>>
...
<div class="wpo-fieldgroup">
	...
	<p>
		...
		<label for="cache_exception_urls"><?php echo wp_kses_post(sprintf(__('List paths (e.g. %s) that should not be cached (one per line)', 'wp-optimize'), '<code>'.esc_html_x('/product/green-beans', 'an example path', 'wp-optimize').'</code>')); ?> </label>
	...
	<span>
		<?php
			...
			esc_html_e('Use the wildcard * to exclude child URLs.', 'wp-optimize'); ?> <?php echo wp_kses_post(sprintf(_x('e.g. %1$s or %2$s', '%s are examples of path using the wildcard *', 'wp-optimize'), '<code>'.esc_html_x('/shop/*', 'an example path with the wildcard (*)', 'wp-optimize').'</code>', '<code>'.esc_html_x('*sample-path*', 'a second example path using the wildcard (*) twice', 'wp-optimize').'</code>'));
		?>
	</span>
...
<div class="wpo-fieldgroup">
	...
	<p>
		...
		<textarea name="cache_ignore_query_variables" id="cache_ignore_query_variables" class="cache-settings" placeholder="wpo_cache_debug"><?php echo esc_textarea($cache_ignore_query_variables); ?></textarea>
	</p>
...
<div class="wpo-fieldgroup">
	...
	<p>
		...
		<?php
			...
			echo '&nbsp;';
			// Translators: %s is a link to the codex.
