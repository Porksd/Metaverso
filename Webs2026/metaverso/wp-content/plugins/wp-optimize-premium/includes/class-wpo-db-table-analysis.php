...
class WPO_DB_Table_Analysis {
	...
	public function __construct() {
		...
		if (true === SAVEQUERIES && version_compare($wp_version, '5.3.0', '>=')) {
			// log_query_custom_data filter available since WordPress 5.3.0
	 ...
	 *
	 * @param array|null  $query_data      Custom query data.
	 * @param string      $query           The query's SQL.
	 * @param float       $query_time      Total time spent on the query, in seconds.
	 * @param string      $query_callstack Comma-separated list of the calling functions.
	 * @return array
	 */
	public function save_plugin_query_filter(?array $query_data, string $query, float $query_time, string $query_callstack): array {
	...
	public function execute_query_analysis() {
		...
		foreach ($this->queries as $plugin_name => $queries) {
			foreach ($queries as $query_data) {
				...
				if ('' !== $table_name) {
					$table_usage[$plugin_name][$table_name]['last_used'] = gmdate("Y-m-d H:i:s");
					...
					$snap_ts_utc = gmdate('Y-m-d H:i:s', $snap_ts);
	...
	public function remove_old_threads($usage) {
		...
		$remove_from = gmdate("Y-m-d H:i:s", time() - $ttl);
	...
	static public function report_query_usage_per_minute($threads) {
		...
		if (0 === count($per_minute)) {
			...
			return (int) round(array_sum($per_minute) / count($per_minute), 0);
		}
	}
	...
	static public function report_query_time_average($data) {
		if (0 === count($data)) {
			return esc_html__("N/A", 'wp-optimize');
		} else {
