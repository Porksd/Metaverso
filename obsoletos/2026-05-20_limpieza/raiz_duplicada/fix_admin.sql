-- FIX SCRIPT: Run this in Supabase SQL Editor
-- This script fixes missing tables and columns for the Admin Panel

-- 1. Create admin_profiles table if it doesn't exist (Fix for 404 error)
CREATE TABLE IF NOT EXISTS admin_profiles (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    email TEXT UNIQUE NOT NULL,
    role TEXT NOT NULL DEFAULT 'editor',
    permissions JSONB DEFAULT '{}',
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);

-- 2. Add default superadmins
INSERT INTO admin_profiles (email, role, permissions)
VALUES 
('apacheco@lobus.cl', 'superadmin', '{"all": true}'),
('admin@metaversotec.com', 'superadmin', '{"all": true}'),
('porksde@gmail.com', 'superadmin', '{"all": true}')
ON CONFLICT (email) DO UPDATE SET 
role = 'superadmin', 
permissions = '{"all": true}';

-- 3. Enable RLS on admin_profiles
ALTER TABLE admin_profiles ENABLE ROW LEVEL SECURITY;

-- 4. Create policies (drop first to avoid errors if they exist)
DROP POLICY IF EXISTS "admin_profiles_read_own" ON admin_profiles;
CREATE POLICY "admin_profiles_read_own" ON admin_profiles FOR SELECT USING (true);

DROP POLICY IF EXISTS "admin_profiles_all_super" ON admin_profiles;
CREATE POLICY "admin_profiles_all_super" ON admin_profiles FOR ALL USING (
    EXISTS (SELECT 1 FROM admin_profiles WHERE email = auth.jwt() ->> 'email' AND role = 'superadmin')
);

-- 5. Add user_registration_config column to companies table (Fix for missing column error)
ALTER TABLE companies
ADD COLUMN IF NOT EXISTS user_registration_config JSONB DEFAULT '{
    "company_collab": { "visible": true, "required": true },
    "job_position": { "visible": true, "required": true },
    "gender": { "visible": true, "required": true },
    "age": { "visible": true, "required": true }
}'::jsonb;

-- Notify success
DO $$
BEGIN
    RAISE NOTICE 'Admin schema fixed successfully.';
END $$;
