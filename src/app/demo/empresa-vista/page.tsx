...
export default function DemoEmpresaPortal() {
    ...
    const cleanWelcomeMessage = (company?.welcome_message || "Muestra a tu equipo una operacion de capacitacion robusta: avance visible, evidencia auditable y foco en desempeno real.")
        .replace(/<[^>]*>/g, "")
        .trim();
    ...
    return (
        <div className="min-h-screen text-white flex flex-col items-center justify-center p-6 md:p-10 font-sans relative overflow-hidden">
            ...
            >
                <source src="/techvideo02.mp4?v=demo-empresa-2" type="video/mp4" />
                <source src="/techvideo01.mov?v=demo-empresa-2" type="video/quicktime" />
            </video>
            ...
            <Link href="/demo" className="fixed top-6 left-6 z-50 flex items-center gap-2 px-4 py-2 rounded-lg bg-white/10 border border-white/20 text-white text-sm font-semibold hover:bg-white/20 transition-colors">
                <ArrowLeft className="w-4 h-4" /> Volver a Presentacion
            </Link>
            ...
            <div className="relative z-10 w-full flex flex-col items-center">
                <div className="max-w-6xl w-full space-y-10">
                    ...
                    <header className="flex flex-col items-center text-center space-y-4 pt-10">
                        ...
                        <h1 className="text-4xl md:text-6xl font-black tracking-tight leading-[0.95]">{company.welcome_title || "Formacion con control"}<span className="block text-brand">resultados y trazabilidad</span></h1>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <Link href="/demo/alumno-login" className="group">
                            <motion.div whileHover={{ y: -5 }} className="glass p-10 h-full border-white/10 hover:border-cyan-300/40 transition-all space-y-6 relative overflow-hidden bg-black/30">
                                ...
                                <div className="space-y-2">
                                    <h3 className="text-2xl font-black uppercase tracking-tight">Gestion de Colaboradores</h3>
                        ...
                        <Link href="/admin/metaverso/encuestas" className="group">
                            <motion.div whileHover={{ y: -5 }} className="glass p-10 h-full border-white/10 hover:border-cyan-300/40 transition-all space-y-6 relative overflow-hidden bg-black/30">
                                ...
                                <div className="space-y-2">
                                    <h3 className="text-2xl font-black uppercase tracking-tight">Reportes e Insights</h3>
                                    <p className="text-white/55 text-sm leading-relaxed">Accede a metricas de satisfaccion, cumplimiento y ROI de capacitacion para decisiones informadas.</p>
                                </div>
                                <div className="flex items-center gap-2 text-cyan-300 text-xs font-black uppercase tracking-widest pt-4">
                    ...
                    <div className="text-center space-y-4">
                        ...
                        <Link href="/demo" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-white/5 border border-white/15 text-white font-bold uppercase tracking-wider text-[11px] hover:border-cyan-400/40 hover:text-cyan-300 transition-colors">
                            <ArrowLeft className="w-4 h-4" /> Volver a Presentacion Comercial
                        </Link>
                    </div>
                </div>
            </div>
