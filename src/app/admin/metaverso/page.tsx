    ...
    const handleSendCompanyReportNow = async () => {
        ...
        try {
            ...
            if (payload?.result?.sent) {
                ...
                fetchCompanies();
            } else if (payload?.result?.reason === 'certificate_limit_reached') {
                const oldestDate = new Date(payload.result.oldestCertificate.generatedAt).toLocaleString('es-CL');
                if (!window.confirm(`Se alcanzó el límite máximo de 10 informes activos.\n\n¿Desea sobreescribir el informe generado el ${oldestDate}?`)) return;

                const overwriteResponse = await fetch('/api/reports/company-progress/send', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
                    body: JSON.stringify({
                        companyId: editingCompany.id,
                        force: true,
                        overwriteCertificateToken: payload.result.oldestCertificate.token,
                        overrides: {
                            includeStudents: editingCompany.report_include_dashboard_body !== false,
                            includePdfAttachment: editingCompany.report_include_pdf_attachment !== false,
                            copyEmails: editingCompany.report_copy_emails_enabled === true ? (editingCompany.report_copy_emails || '') : ''
                        }
                    })
                });
                const overwritePayload = await overwriteResponse.json();
                if (overwritePayload?.result?.sent) {
                    alert('Informe enviado y el más antiguo fue reemplazado correctamente.');
                    fetchCompanies();
                } else {
                    alert(overwritePayload?.error || 'No se pudo sobreescribir el informe.');
                }
            } else {
    ...
    if (isAuthorized === false) return (
        <div className="min-h-screen bg-black flex flex-col items-center justify-center p-8 text-center space-y-6">
            ...
            <div className="flex gap-3">
                ...
                <button onClick={async () => { await supabase.auth.signOut(); router.push("/admin/metaverso/login"); }} className="bg-red-500/20 border border-red-500/30 text-red-400 px-8 py-4 rounded-2xl font-black uppercase text-xs hover:bg-red-500/30 transition-all">Cerrar Sesión</button>
            </div>
        </div>
    );
