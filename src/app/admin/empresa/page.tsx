                                        ...
                                        return validEnrollments.map((en: any, idx: number) => {
                                            ...
                                            return (
                                                <tr key={`${st.id}-${en.course_id}`} className="hover:bg-white/[0.02] text-sm font-medium">
                                                    ...
                                                    <td className="px-6 py-4">
                                                        {isCompleted ? (() => {
                                                            const certF = courseCertFlags[en.course_id] || { participacion: true, aprobacion: false };
                                                            const fetchStudentComp = async () => {
                                                                if (!companyId) return null;
                                                                const [{ data: comp }, { data: studentData }] = await Promise.all([
                                                                    supabase.from('companies').select('*').eq('id', companyId).single(),
                                                                    supabase.from('students').select('digital_signature_url, age, gender, company_name, job_position').eq('id', st.id).single()
                                                                ]);
                                                                        ...
                                                                        <button
                                                                            onClick={async () => {
                                                                                ...
                                                                                if (r?.comp) setCertData({
                                                                                    ...
                                                                                    studentSignature: r.studentData?.digital_signature_url,
                                                                            ...
                                                                            className="p-2 rounded-lg bg-green-500/10 text-green-400 text-[10px] font-black flex items-center gap-1 border border-green-500/30 hover:bg-green-500 hover:text-black transition-all"
